...
## 5. M7 — BUG-A/B/C 修复对账 (2026-08-29 01:43, 基线 c72164e 后)

> 本节为 M6 终局对账表的增补，修复 M6 报告中 3 项虚高（KG 500 节点、taint 三步链、五服务自愈）并以功能面取证为准（管理面状态不作为完成证据）。

| # | Task | 判据 | 取证命令 | 实际输出 | 状态 |
|---|---|---|---|---|---|
| M7-2 | **服务真接管端口** | 5 端口各 1 LISTEN，父进程均为 nssm.exe；health 200 | `Get-NetTCPConnection -LocalPort 8080,8090,8901,8902,8903` <br> `Invoke-WebRequest http://127.0.0.1:8090/healthz` | `8080 pid=52576 parent=nssm` `8090 pid=52576 parent=nssm` `8901 pid=32304 parent=nssm` `8902 pid=25732 parent=nssm` `8903 pid=42004 parent=nssm`；`healthz ok` `8901 ok` `8902 {"node_count":371,"graph_ready":true}` `8903 ok`；`svcctl.log: ports still listening after cleanup: 0` `LastTaskResult=0` | ✅ |
| M7-3 | **重建网关加载 taint 新逻辑** | `go build` 0，日志无 bind 错误，`replayed 156 events` | `go build -o bin/gateway.exe ./cmd/gateway` <br> `cat logs/ASG-Gateway.log` | `BUILD_EXIT:0`；`[kg] replayed 156 events into graph` `node_count=371 edge_count=468` `graph_ready=true`；无 `bind:` 错误 | ✅ |
| M7-4 | **三步链 BLOCK（taint 轴）** | Read→base64→curl 三步中 step3 必须 BLOCK 且 reason 含 taint 血缘；正常域名变体仍 BLOCK；干净 session 同 URL 不 BLOCK；asg-guard 同步 deny | `curl -X POST /api/activity` 三步链（session chain-live-1） | `step1 {"status":"ok"}` `step2 {"verdict":"CONFIRM"}` `step3 {"verdict":"BLOCK","code":"SENSITIVE_OP_BLOCK","message":"behavior: value '/tmp/x' in Bash originated from untrusted source derived(Read:/home/u/.aws/credentials) (data-flow taint)"}`；`variant api.company-cdn.io → BLOCK`（同 reason，证明 taint 而非正则）；`clean-session same URL → CONFIRM`（证明拦来源非 URL）；`asg-guard → {"permissionDecision":"deny"}` | ✅ |
| M7-5 | **误报回归 0/6** | 已读密钥的同一 session 内 6 个良性操作均不得 BLOCK | `Read /srv/app/README.md` `Bash ls -la` `Bash git status` `Bash npm run build` `WebFetch go.dev` `Write notes.md` | 全部 `verdict != BLOCK`（`ok` 或 `CONFIRM`）；`0/6`；`go test ./... ok` `asg-guard_test.sh PASS=*** FAIL=0`。实际执行用例（与 plan 等价替换）：`Read /srv/app/README.md`（替 `Read /etc/hosts`）、`Bash npm run build`（替 `Bash npm test`），其余 4 项一致 | ✅ |
| M7-6 | **KG 自愈** | kill 8902 后 `graph_ready:false`，自愈后 `node_count>0` 且 ≥ kill 前基线（期间新事件并入，故不必相等），gateway 日志含 `re-ingesting N events` 且 gateway PID 未变（证明自愈非重启） | `taskkill /F /PID 32788` <br> `curl 8902/health` 立即 <br> `curl /api/kg/graph/nodes` 前后 | `baseline 342` → kill 后 `health {"node_count":0,"graph_ready":false}`（诚实）→ `nssm restart` 后 `health {"pid":736,"node_count":0}` → 自愈后 `371`；`gateway.log: [kg] worker graph empty (node_count=0), re-ingesting 156 events` `self-heal complete: node_count=371`；`gateway PID 45896 未变` | ✅ |
| M7-7 | **四服务 kill→30s 恢复** | 逐个 kill wrapper+`sc start`，计时到 health 200 | `kill_test2.ps1` via `ASG-ServiceControl` | `ASG-Gateway 4.4s` `ASG-Behavior 4.4s` `ASG-KGWorker 12.5s` `ASG-OutputGuard 4.4s` 全部 ≤30s，父进程均为 nssm | ✅ |
