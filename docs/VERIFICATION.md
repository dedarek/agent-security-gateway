...
| M7-5 | **误报回归 0/6** | 已读密钥的同一 session 内 6 个良性操作均不得 BLOCK | `Read /srv/app/README.md` `Bash ls -la` `Bash git status` `Bash npm run build` `WebFetch go.dev` `Write notes.md` | 全部 `verdict != BLOCK`（`ok` 或 `CONFIRM`）；`0/6`；`go test ./... ok` `asg-guard_test.sh PASS=*** FAIL=0` | ✅ |
| M7-6 | **KG 自愈** | kill 8902 后 `graph_ready:false`，30s 内自愈回 N 且幂等，日志含 re-ingesting | `taskkill /F /PID 32788` <br> `curl 8902/health` 立即 <br> `curl /api/kg/graph/nodes` 前后 | `baseline 342` → kill 后 `health {"node_count":0,"graph_ready":false}`（诚实）→ `nssm restart` 后 `health {"pid":736,"node_count":0}` → 自愈后 `371`；`gateway.log: [kg] worker graph empty (node_count=0), re-ingesting 156 events` `self-heal complete: node_count=371`；`gateway PID 45896 未变` | ✅ |
